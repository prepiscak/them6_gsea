...
#' @param ADD
#' @return ADD
create_rnk_list <- function(data_name=NULL, raw_datasets=NULL){
  import::here("%>%", .from = magrittr)
  import::here(select, filter, rename, arrange, desc, group_by, summarise, ungroup, .from = dplyr)
  
  # selecting raw_data subset
  raw_data <- raw_datasets[[data_name]]
  
  # cleaning raw_data
  clean_data <- raw_data %>%
    rename(gene_symbol = Gene.Name.HGNC,
    ...
    summarise(log2FC = log2FC[abs(log2FC) == max(abs(log2FC))]) %>%  # duplicated gene_symbols keep one max(abs(log2FC))
